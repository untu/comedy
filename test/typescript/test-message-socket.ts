...
import { MessageSocket } from '../../lib/net/message-socket';
let server: Server;
let client: Socket;
 ...
 */
function splitBuffer(buf: Buffer, nChunks: number) {
  let chunkLength = Math.trunc(buf.length);
