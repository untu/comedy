...
var BalancerActor = require('./balancer-actor.js');
...
class CustomBalancerActor extends BalancerActor {
   ...
   * - {Object} definition Balancer implementation definition.
  ...
  async _forward(methodName, args) {
    if (!this.childrenById) {
      var children = await P.all(this._children());

      this.childrenById = _.indexBy(children, child => child.getId());
    }

    if (this.impl.clusterChanged) {
      var childrenState = _.reduce(this.childrenById, (memo, child) => {
    ...
    var childId = await P.resolve(this.impl.forward(...args));
    var child = this.childrenById[childId];
