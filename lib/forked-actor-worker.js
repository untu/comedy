...
process.once('message', (msg, handle) => {
  ...
  var resources = msg.body.resources &&
    _.map(msg.body.resources, (resource, idx) => {
  ...
  if (msg.body.marshaledCustomParameters) {
    _.each(msg.body.marshaledCustomParameters, (value, key) => {
      if (value.marshallerType == '_socketHandle') {
        ...
        throw new Error(`Unknown marshaller type for custom parameter ${key}: ${value.marshallerType}`);
      }
    });
  }
