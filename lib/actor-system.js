...
class ActorSystem {
  ...
  constructor(options = {}) {
    ...
    if (options.root) {
      ...
      this.rootActorPromise = P.resolve()
        .then(() => {
          if (options.rootActorConfig.customParameters && options.rootParametersMarshalledTypes) {
    ...
    return P.resolve()
      ...
      .then(Definition0 => {
        ...
        switch (config.mode || 'in-memory') {
          case 'in-memory':
            return this._createInMemoryActor(Definition0, parent, _.defaults({ name: actorName }, config));
    ...
    return P.resolve()
      ...
      .then(definition => {
        ...
        if (this.options.test && config.clusterSize > 1) {
          return this._createBalancerActor(parent, config)
            .then(balancerActor => {
        ...
        return new ClientActorProxy(new InMemoryActor({
          system: this,
          parent: parent,
  ...
  async _createForkedActor(definition, parent, config = {}) {
    ...
    if (config.clusterSize > 1) {
      let balancerActor = await this._createBalancerActor(parent, config);
   ...
   * @private
   */
  async _createBalancerActor(parent, config) {
