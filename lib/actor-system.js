...
class ActorSystem {
  ...
  constructor(options) {
    ...
    var contextPromise = this._createContext(this.contextDefinition)
      .then(context => {
    ...
    return P.resolve()
      ...
      .then(Definition0 => {
        ...
        switch (options.mode || 'in-memory') {
          case 'in-memory':
            return this.createInMemoryActor(Definition0, parent, _.defaults({ name: actorName }, options));

          case 'forked':
            return this.createForkedActor(Definition, parent, _.defaults({ name: actorName }, options));
   ...
   */
  createInMemoryActor(Definition, parent, options) {
    return P.resolve()
      .then(() => {
   ...
   */
  createForkedActor(definition, parent, options) {
    options = options || {};
        ...
        return new P((resolve, reject) => {
          var createMsg = {
            ...
            body: {
              ...
              context: _.isString(this.contextDefinition) ? this.contextDefinition :
                this._serializeDefinition(this.contextDefinition),
              contextFormat: _.isString(this.contextDefinition) ? 'modulePath' : 'serialized',
              config: this.config,
          ...
          workerProcess.once('exit', () => {
            ...
            if (options.onCrash == 'respawn' && !this.destroying && !actor.isDestroying()) {
              ...
              this.createForkedActor(definition, parent, _.extend({}, options, { id: actor.getId() }))
  ...
  destroy() {
    ...
    this.destroyPromise = this.rootActorPromise
      ...
      .then(() => {
        return P.map(_.values(this.resourcePromises), resource => {
          if (_.isFunction(resource.destroy)) {
  ...
  /**
   * Determines a given defintion name.
   *
    ...
    if (resourceNames && _.isFunction(Definition)) {
      return P
        .map(resourceNames, resourceName => {
          if (this.resourcePromises[resourceName]) return this.resourcePromises[resourceName];

          var resourcePromise = P.resolve()
            .then(() => {
              // Attempt to find a resource definition.
              var ResourceDef = this.options.resources &&
                _.find(this.options.resources, ResourceDef0 => {
                  var resourceName0 = this._resourceName(ResourceDef0);

                  return resourceName0 == resourceName;
                });

              if (ResourceDef) {
                var resourceInstance = new ResourceDef();

                if (_.isFunction(resourceInstance.initialize)) {
                  return P.resolve()
                    .then(() => resourceInstance.initialize(this))
                    .then(() => resourceInstance.getResource());
                }
                else {
              ...
              if (this.context) {
                return this._readProperty(this.context, resourceName);
              }
            })
            .tap(resource => {
              if (!resource) {
          ...
          this.resourcePromises[resourceName] = resourcePromise;
