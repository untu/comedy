...
class ActorSystem {
    ...
    return P.resolve()
      .then(() => {
        ...
        return new P((resolve, reject) => {
          var createMsg = this._generateActorCreationMessage(definition, options);
  ...
  _createRemoteActor(definition, parent, options) {
    ...
    if (!host) return P.reject(new Error('Required option "host" missing for "forked" mode.'));

    return P.fromCallback(cb => {
      var socket = new MessageSocket(port, host);

      socket.on('error', cb);
      socket.on('connect', () => {
        var createMsg = this._generateActorCreationMessage(definition, options);
        var createMsgData = JSON.stringify(createMsg);
        var createMsgBuf = Buffer.alloc(1 + 4 + createMsgData.length);

        createMsgBuf.writeUInt8(1, 0); // JSON message type.
        createMsgBuf.writeUInt32BE(createMsgData.length, 1); // Message length.
        createMsgBuf.write(createMsgData, 1 + 4);

        socket.write(createMsgBuf, err => {
          ...
          socket.once('message', msg => {
            ...
            if (msg.type != 'actor-created' || !msg.body || !msg.body.id || !msg.body.port)
              return cb(new Error('Unexpected response for "create-actor" message.'));

            // Now connect to newly-created actor.
            var actorSocket = new MessageSocket(msg.body.port, host);

            actorSocket.on('error', cb);
   ...
   * @private
   */
  _generateActorCreationMessage(definition, options = {}) {
    var createMsg = {
      type: 'create-actor',
      body: {
