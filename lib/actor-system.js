...
class ActorSystem {
    ...
    return P.resolve()
      .then(() => {
        ...
        if (_.isFunction(Definition)) {
          def = this._injectResources(Definition);
        }

        return new InMemoryActor(this, parent, def, options.name, options.customParameters);
      });
  }
   ...
   * @returns {*} Definition instance with injected resources.
   * @private
   */
  _injectResources(Definition) {
    ...
    // Resource injection.
    if (_.isArray(resourceNames) && _.isFunction(Definition)) {
      // Read resource list.
      ...
      return new Definition(...resources);
    }
