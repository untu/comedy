...
class ActorSystem {
  ...
  constructor(options = {}) {
    ...
    this.log = common.isPlainObject(options.loggerConfig) ?
      Logger.fromConfigurationObject(options.loggerConfig) : Logger.fromConfigurationFile(options.loggerConfig);

    if (options.logger) {
      let loggerImpl = this._instantiate(options.logger);
  ...
  createActor(Definition, parent, options = {}) {
    return P.resolve()
      ...
      .then(Definition0 => {
        let actorName = options.name || this._actorName(Definition0);
        ...
        switch (options.mode || 'in-memory') {
          ...
          case 'threaded':
            return P.resolve(this._createThreadedActor(Definition, parent, _.defaults({ name: actorName }, options)));
      ...
      .then(definition => {
        ...
        if (this.options.test && options.clusterSize > 1) {
          return this._createBalancerActor(parent, options)
            .then(balancerActor => {
              let childPromises = _.times(options.clusterSize, () =>
  ...
  async _createForkedActor(definition, parent, options = {}) {
    ...
    if (options.clusterSize > 1) {
      let balancerActor = await this._createBalancerActor(parent, options);
  ...
  async _createRemoteActor(definition, parent, options) {
    ...
    if (clusterDef) {
      let balancerActor = await this._createBalancerActor(parent, options);
      let clusterSize = options.clusterSize || clusterDef.length;
   ...
   * @private
   */
  async _createThreadedActor(definition, parent, options = {}) {
    // Perform clusterization, if needed.
    if (options.clusterSize > 1) {
      let balancerActor = await this._createBalancerActor(parent, options);
    ...
    let actor =
      new ThreadedActorParent({ system: this, parent: parent, definition: definition, additionalOptions: options });
  ...
  async _createBalancerActor(parent, options) {
    if (!options.balancer || options.balancer == 'round-robin') {
      ...
      let balancerDef = this.balancers[options.balancer];
