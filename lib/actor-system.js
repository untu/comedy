...
class ActorSystem {
    ...
    return P.resolve()
      .then(() => {
        ...
        return new P((resolve, reject) => {
          var createMsg =
            this._generateActorCreationMessage(definition, parent, _.defaults({ mode: 'forked' }, options));
  ...
  _createRemoteActor(definition, parent, options) {
    return new P((resolve, reject) => {
      ...
      if (cluster) {
        var clusterDef = this.options.clusters[cluster];

        if (!clusterDef) return reject(new Error(`Cluster with name "${cluster}" is not defined.`));

        var balancerActor = new RoundRobinBalancerActor(this, parent, options.name);

        var childPromises = _.map(clusterDef, host =>
          balancerActor.createChild(definition, _.chain(options).omit('cluster').extend({ host: host }).value()));
      ...
      socket.on('connect', common.guard(reject, () => {
        var createMsg = this._generateActorCreationMessage(definition, parent, _.defaults({ mode: 'remote' }, options));
   ...
   * @private
   */
  _initializeResource(resourceName) {
        ...
        if (ResourceDefCls) {
          ...
          if (_.isFunction(ResourceDefCls.inject)) {
            depsPromise = P.map(
              ...
              resourceDep => this._initializeResource(resourceDep).then(resourceDef => {
