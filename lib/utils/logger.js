...
class LoggerConfiguration extends EventEmitter {
  ...
  static fromFiles(paths = []) {
    ...
    paths.forEach(path => {
      try {
        ...
        data = common.deepExtend(JSON.parse(loggerConf), data);
    ...
    paths.forEach(path => {
      ...
      fs.watchFile(path, curStats => {
        ...
        P
          ...
          .then(
            results => _.compact(results).reduce((acc, cur) => {
              try {
                acc = common.deepExtend(JSON.parse(cur), acc);
              }
              catch (error) {
  ...
  _setConfiguration(data) {
    if (typeof data.categories !== 'object') {
      throw new Error(`Invalid logger configuration format in: ${JSON.stringify(data)}`);
    }
