...
class LoggerConfiguration extends EventEmitter {
  ...
  static fromFiles(paths = [], log) {
    ...
    var data = _.reduce(paths, (memo, path) => {
      try {
        var loggerConf = fs.readFileSync(path);
    ...
    var configuration = new LoggerConfiguration(data);
    ...
    paths.forEach(path => {
      ...
      fs.watchFile(path, curStats => {
        ...
        P
          ...
          .then(configs => {
            var data = _.compact(configs).reduce((memo, cur) => common.deepExtend(cur, memo), {});
  ...
  _setConfiguration(data) {
    ...
    if (!this.current || typeof this.current.categories !== 'object') {
      this.current = _.extend(this.current, { categories: { 'Default': logLevels.Info } });
    }
...
class Logger {
  ...
  static fromConfigurationFile(path = [], category = 'Default') {
    var configuration = LoggerConfiguration.fromFiles(_.isArray(path) ? path : [path], new Logger());
