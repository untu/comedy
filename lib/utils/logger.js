      ...
      fs.watchFile(path, curStats => {
        ...
        Promise.all(
          ...
          results => results.filter(val => val).reduce((acc, cur) => {
            try {
              acc = _.mapObject(JSON.parse(cur), function(val, key) {
                return _.extend(val, acc[key]);
              });
            }
            catch (error) {
...
class Logger {
  ...
  /**
   * Initializes configuration emitter from provided files paths and creates new logger instance subscribed to it.
   * 
   * @param {String[]} paths Paths to logger configuration files.
   * @param {String} [category='Default'] Logger category.
   * @returns {Logger} Logger instance.
   */
  static fromConfigurationFiles(paths, category = 'Default') {
    const configuration = LoggerConfiguration.fromFiles(paths);
    let level;
   ...
   *
   * @param {LoggerConfigration} configuration Logger configuration instance.
   * @private
   */
