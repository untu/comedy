...
class ForkedActor extends Actor {
  ...
  sendBusMessage(event, ...args) {
    super.sendBusMessage(event, ...args);
    this._send0({ type: 'system-bus-message', event, senders: [this.getId()], args });
  }
  ...
  transmitBusMessage(event, senders = [], ...args) {
    ...
    if (!this.hasParent()) {
      this.getBus().emit(event, senders.concat(this.getId()), ...args);
    }
    ...
    this.bus.on('message', (msg, handle) => {
      ...
      if (msg.type == 'actor-tree') {
        ...
        if (this.hasParent() &&
          ...
          this.getBus().emit(msg.event, msg.senders, ...msg.args);
        }
      }
      else {
