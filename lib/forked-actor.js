...
class ForkedActor extends Actor {
  ...
  /**
   * Send bus message to a forked actor.
   * 
   * @param {String} event Event name.
   * @param {String[]} senders Chain of actors (ID's) who sent that message.
   * @param {any} args Emission arguments.
   */
  sendBusMessage(event, senders, ...args) {
    this._send0({ type: 'system-bus-message', event, senders, args });
  }
  ...
  _setBus(bus) {
    ...
    this.bus.on('message', (msg, handle) => {
      ...
      if (msg.type == 'actor-tree') {
        ...
        this.getSystem().bus.emit(msg.event, msg.senders, ...msg.args);
      }
      else {
