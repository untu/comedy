...
class Actor extends EventEmitter {
  ...
  constructor(options) {
    ...
    if (common.isPlainObject(options.definition)) {
      ...
      this.origDefinition = _.clone(options.definition);
    }
    else {
  ...
  async changeConfiguration(config = { mode: 'in-memory' }) {
    ...
    let newActor = await this.system._createActor(this.origDefinition || this.definition, this.parent, config);
  ...
  async changeGlobalConfiguration(config) {
    let self = this;
    let selfConfig = config[this.getName()];

    if (selfConfig) {
      self = await this.changeConfiguration(selfConfig);
    }

    await P.map(self.childPromises, child => child.changeGlobalConfiguration(config));
  }
