...
class Actor extends EventEmitter {
   ...
   * - {Boolean} [dropMessagesOnOverload] Whether to drop messages on process overload.
   */
  constructor(options) {
    super();
    ...
    this.state = 'new';
    this.dropMessagesOnOverload = options.dropMessagesOnOverload;
  }
  ...
  createChild(behaviour, options) {
    ...
    let log = this.getLog();
    let childPromise = this.system.createActor(behaviour, this._parentReference(), options)
  ...
  /**
   * Sends a bus message to this actor's process system bus instance.
   * 
   * @param {String} event Event name.
   * @param {any} args Emission arguments.
   */
  sendBusMessage(event, ...args) {
    this.getBus().emit(event, [this.getId()], ...args);
  }
   ...
   */
  _checkOverload() {
    if (this.dropMessagesOnOverload && this.system.isOverloaded()) {
